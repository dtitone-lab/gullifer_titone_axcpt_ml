...
```{r}
rt.sd <- axcpt_ppd %>% 
  group_by(participant, L2_current_exposure, aoa.x, PCA_General, PCA_Work, Condition) %>% 
  summarise(meanRT=mean(RT_correct, na.rm=T), 
...
p1 <- ggplot(rt.sd, aes(x=aoa.x, y = meanRT, colour=Condition))+ geom_smooth(method="lm") +
  geom_point(alpha=.2) +
...
p1 <- ggplot(raw, aes(x=aoa.x, y = RT_correct, colour=Condition))+ 
  geom_point(alpha=.05) +
...
p1 <- ggplot(acc.sd, aes(x=aoa.x, y = meanACC, colour=Condition))+ geom_smooth(method="lm") +
  geom_point(alpha=.2) +
...
mod.rt.0  <- "RT_correct  ~  Condition + occupation + (1|participant)"
...
## RT models students
```{r}
axcpt_ppd_students <- axcpt_ppd[axcpt_ppd$occupation=="student",]

#axcpt_ppd <- axcpt_ppd[!(axcpt_ppd$participant %in% exclude),]
axcpt_ppd_students$Condition<-relevel(axcpt_ppd_students$Condition, ref="BX")
all.rt.0    <- lmer(mod.rt.0, axcpt_ppd_students, REML=F)
all.rt.1    <- lmer(mod.rt.1, axcpt_ppd_students, REML=F)
all.rt.1.1  <- lmer(mod.rt.1.1, axcpt_ppd_students, REML=F)
all.rt.2    <- lmer(mod.rt.2, axcpt_ppd_students, REML=F)
all.rt.3    <- lmer(mod.rt.3, axcpt_ppd_students, REML=F)
```
...
## RT models workers
```{r}
axcpt_ppd_workers <- axcpt_ppd[axcpt_ppd$occupation=="work",]

#axcpt_ppd <- axcpt_ppd[!(axcpt_ppd$participant %in% exclude),]
axcpt_ppd_workers$Condition<-relevel(axcpt_ppd_workers$Condition, ref="BX")
all.rt.0    <- lmer(mod.rt.0, axcpt_ppd_workers, REML=F)
all.rt.1    <- lmer(mod.rt.1, axcpt_ppd_workers, REML=F)
all.rt.1.1  <- lmer(mod.rt.1.1, axcpt_ppd_workers, REML=F)
all.rt.2    <- lmer(mod.rt.2, axcpt_ppd_workers, REML=F)
all.rt.3    <- lmer(mod.rt.3, axcpt_ppd_workers, REML=F)
```
...
```{r}
axcpt_ppd <- axcpt_ppd %>%  
  mutate(RTbin = cut(Target.RT, breaks = seq(100,1000,200)))

axcpt_ppd %>% 
  drop_na(RTbin) %>% 
  group_by(participant, RTbin, Condition) %>%
  summarise(meanAcc=mean(accuracy_target, na.rm=T), N=n()) %>% 
  ungroup() %>% 
  group_by(RTbin, Condition) %>% 
  summarise(sem = sd(meanAcc) / sqrt(n()), meanAcc=mean(meanAcc)) %>% 
  ggplot(aes(x=RTbin, y=meanAcc, colour=Condition, 
             group=Condition, ymin=meanAcc-1.96*sem, ymax=meanAcc+1.96*sem)) + geom_pointrange() 
           
           
axcpt_ppd %>% 
  drop_na(RTbin) %>% 
  group_by(participant, RTbin, Condition, PCA_General, PCA_Work) %>% 
  summarise(meanAcc=mean(accuracy_target, na.rm=T), N=n()) %>% 
  ggplot(aes(x=PCA_General, y=meanAcc, colour=Condition, group=Condition)) + geom_point() + facet_wrap(~RTbin) + geom_smooth()
```


## Task version checking and ID
```{r}
axcpt_ppd$ExperimentName <- relevel(factor(axcpt_ppd$ExperimentName), ref="AX-CP-ORIG100")
x <- lmer(RT_correct ~ Condition * ExperimentName + (1+Condition|participant), axcpt_ppd, REML=F)
anova(x)
summ(x)


p <- axcpt_ppd %>%  
  group_by(participant,ExperimentName) %>%  
  summarise(PCA_G = mean(PCA_General), 
            PCA_W = mean(PCA_Work), 
            aoa=mean(aoa), 
            L2_exposure = mean(L2_exposure))

anova(lm(PCA_G ~ ExperimentName,p))
anova(lm(PCA_W ~ ExperimentName,p))
summ(lm(PCA_W ~ ExperimentName,p), confint=T)
anova(lm(aoa ~ ExperimentName,p))
anova(lm(L2_exposure ~ ExperimentName,p))
